...
rusInsectImages = struct('fileName', 'None', 'labels', zeros(178,1,"logical"), 'isInsect', 'None', 'insectRows', []);
i = 1;
x = 1;
rusInsectImageFilesArray = string.empty;
predictedLabels = [];
...
for k = 1:length(files)
    ...
    for imageIdx = 1:numel(matData.adjusted_data_decembercal)
        ...
        if insectDetected
            ...
            if i == 1
                rusInsectImages.fileName = imageFilename;
                rusInsectImages.labels = labels;
                rusInsectImages.isInsect = insectDetectedStr;
                rusInsectImages.insectRows = insect_rows;
                i = 2;
            else
                rusInsectImages(i).fileName = imageFilename;
                rusInsectImages(i).labels = labels;
                rusInsectImages(i).isInsect = insectDetectedStr;
                rusInsectImages(i).insectRows = insect_rows
                i = i + 1;
            end
        end
    end
